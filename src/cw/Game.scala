...
class Game(wall: List[(Int, Int)], coin: List[(Int, Int, Int)], initialX: Int, initialY: Int) {
   ...
   */
  def moveUp(): Unit = {
    if (positionX > 0 && board(positionX)(positionY - 1) != 0) {
      positionY -= 1
      checkCoin()
      checkCoins()
    }
  ...
  def moveLeft(n: Int): Unit = {
    for (_<- 1 to n)
      moveLeft() //Implementation of above moveLeft function which goes through if statement,check coin and coins
  }
  ...
  def moveRight(n: Int): Unit = {
    for (_<- 1 to n)
      moveRight() //Implementation of above moveRight function which goes through if statement,check coin and coins
  }
  ...
  def moveUp(n: Int): Unit = {
    for (_<- 1 to n)
      moveUp() //Implementation of above moveUp function which goes through if statement,check coin and coins
  }
  ...
  def moveDown(n: Int): Unit = {
    for (_<- 1 to n)
      moveDown() //Implementation of above moveDown function which goes through if statement,check coin and coins
  }
  ...
  def move(s: String): Unit = {
    for (moving <- s){
      moving match{
        case 'w' => if (positionY > 0 && board(positionX)(positionY - 1) != 0) positionY -= 1
        case 'a' => if (positionX > 0 && board(positionX - 1)(positionY) != 0) positionX -= 1
        case 's' => if (positionY < 9 && board(positionX)(positionY + 1) != 0) positionY += 1
        case 'd' => if (positionX < 9 && board(positionX + 1)(positionY) != 0) positionX += 1
        case _ =>
      }
      checkCoin()
    }
   ...
   */
  def maxPoints(): Int = {
    var remainingCoins = points
    for {
      i <- 0 until board.length
      j <- 0 until board.length
      if board(i)(j) > 0
    } remainingCoins = remainingCoins + board (i)(j)
    remainingCoins
  }
  ...
  def checkCoins(): Unit = {
    if (saveX != -1){
      val minimumOfX = math.min(positionX, saveX)
      val maximumOfX = math.max(positionX, saveX)
      val minimumOfY = math.min(positionY, saveY)
      val maximumOfY = math.max(positionY, saveY)

      var CollectedCoins = 0
      for {
        i <- minimumOfX to maximumOfX
        j <- minimumOfY to maximumOfY
        if board(i)(j) > 0
      } {
        CollectedCoins = CollectedCoins + board (i)(j)
        board (i)(j) > 0
      }
      points = points + CollectedCoins
      saveX = -1
      saveY = -1
    }
  }
  ...
  def suggestSolution(): String = {
    ...
    def PositionReset(x: Int, y: Int): Unit = {
      positionX = x
      positionY = y
    }
    ...
    for (i <- 0 until board.length) {
      for (j <- 0 until board.length) {
        if (board(i)(j) > 0) {
          val moves = suggestMove(x,y)
          if (moves.isEmpty) {
            PositionReset(x,y)
            return ""
          } else {
            suggestedMove = suggestedMove + move
          }
        }
      }
    }
  ...
  def suggestMove(x: Int, y: Int): String = {
    ...
    var PresentX = positionX
    var PresentY = positionY
    var StepMovements = ""

    while (PresentX != x || PresentY != y){
      val X = x - PresentX
      val Y = y - PresentY

      if (X > 0 && board(PresentX + 1)(PresentY) != 0){
        StepMovements += 'd'
        PresentX += 1
      } else if (Y > 0 && board(PresentX)(PresentY + 1) != 0) {
        StepMovements += 's'
        PresentY += 1
      } else if (X < 0 && board(PresentX - 1)(PresentY) != 0) {
        StepMovements += 'a'
        PresentX -= 1
      } else if (Y < 0 && board(PresentX)(PresentY - 1) != 0){
        StepMovements += 'w'
